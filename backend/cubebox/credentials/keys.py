...
def parse_vault_keys(raw: str) -> list[bytes]:
    ...
    if not keys:
        raise ValueError("Invalid CUBEBOX_AUTH__VAULT_KEY: at least one Fernet key is required")

    for key in keys:
        try:
            Fernet(key)
        except ValueError as exc:
            raise ValueError("Invalid CUBEBOX_AUTH__VAULT_KEY: expected Fernet key(s)") from exc
