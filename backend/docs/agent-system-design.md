...
Live steering enters through `ExecutionSession.submit_input`. Steers received while the host is still preparing an owned attempt wait in a bounded host buffer and are submitted when `AgentStart` opens Session admission. A bounded cancellation tombstone prevents a cancel that arrives first from being lost when its steer is buffered later. Durable steering is acknowledged only from an `InputCommitted` event whose durability is `checkpoint`; an in-memory admission receipt alone does not make a database row injected. The acknowledgement carries its immutable workspace scope so it can reconcile the durable row even when HITL auto-detach has already removed the live Session registration. When a local Session has closed, the host forwards steer and steer-cancellation requests through pub/sub because a replacement worker may own the same resumed run. A closed registration, or the current task during its no-Agent teardown window, rejects admission only when its local resume claim still matches the distributed claim; an older worker stays silent after a replacement takes ownership. Cancellation uses `cancel_input`, while hard run cancellation remains task cancellation.
