...
5. CubeLoop returns an explicit `ExecutionResult`; the host maps its outcome and durable checkpoint facts to completed, cancelled, failed, or paused run state. A checkpoint-durable HITL suspension remains resumable when host event projection fails; the delivery diagnostic is retained and logged instead of stranding the pending request in an error state.
