...
def test_grant_expired_without_refresh_reports_grant_expired() -> None:
    result = compute_effective_state(
        _input(
            credential_policy="org",
            grant=MCPGrantInput(
                scope="org", status="expired", has_refresh=False, auth_method="static"
            ),
        )
    )
    assert result.usable is False
...
def test_grant_expired_with_refresh_reports_grant_expired() -> None:
    result = compute_effective_state(
        _input(
            credential_policy="org",
            grant=MCPGrantInput(
                scope="org", status="expired", has_refresh=True, auth_method="static"
            ),
        )
    )
    assert result.usable is False
...
def test_valid_user_grant_is_usable() -> None:
    result = compute_effective_state(
        _input(
            credential_policy="user",
            grant=MCPGrantInput(
                scope="user", status="valid", has_refresh=True, auth_method="oauth"
            ),
        )
    )
    assert result.usable is True
...
def test_no_auth_install_skips_credential_policy_check() -> None:
    """``auth_required=False`` short-circuits before grant resolution even
...
# ---- Five new cases from Task 6 brief ----


def test_org_disabled_vetoes_everything() -> None:
    """org_disabled=True vetoes even an otherwise fully-valid install."""
    result = compute_effective_state(
        _input(
            org_disabled=True,
            auth_required=True,
    ...
    assert result.usable is False
    assert result.reason == "template_disabled_in_org"
    assert result.credential_availability == "missing"


def test_auth_not_required_is_usable_without_grant() -> None:
    """auth_required=False → usable with credential_availability='not_required'."""
    result = compute_effective_state(
        _input(
            auth_required=False,
            grant=None,
...
def test_missing_grant_oauth_supported_org_policy_is_pending_oauth() -> None:
    """auth_required=True, oauth_supported=True, grant=None, policy='org' → pending_oauth."""
    result = compute_effective_state(
        _input(
            auth_required=True,
            oauth_supported=True,
...
def test_missing_grant_static_only_is_missing_org_grant() -> None:
    """oauth_supported=False → reason 'missing_org_grant', never 'pending_oauth'."""
    result = compute_effective_state(
        _input(
            auth_required=True,
            oauth_supported=False,
...
def test_user_policy_missing_grant_is_user_needs_connection_new() -> None:
    """User policy + missing grant → 'user_needs_connection' regardless of oauth support."""
    result = compute_effective_state(
        _input(
            auth_required=True,
            oauth_supported=True,
