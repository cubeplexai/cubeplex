"""Tests for the request-body → cubeloop.UserMessage builder.

Outbound wire conversion was removed when cubeplex aligned to cubeloop's
...
def test_wire_input_to_user_message_simple_text() -> None:
    msg = wire_input_to_cubeloop_user_message("hello world")
...
def test_wire_input_carries_attachments_in_metadata() -> None:
    ...
    msg = wire_input_to_cubeloop_user_message("look at this", attachments=attachments)
...
def test_wire_input_carries_memory_snapshot() -> None:
    snap = {"id": "m1", "items": []}
    msg = wire_input_to_cubeloop_user_message("hi", memory_snapshot=snap)
