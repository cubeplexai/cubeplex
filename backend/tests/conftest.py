...
#   CUBEPLEX_OBJECTSTORE__ENDPOINT=http://127.0.0.1:9010
...
#
# - MAIN repo: pin the shared `cubeplex_test` DB.
# - WORKTREE: `.worktree.env` only declares the per-slot DEV db
#   (CUBEPLEX_DATABASE__NAME=cubeplex_<slug>), which `cubeplex.config` would load via
#   load_dotenv and run the suite against the dev DB. Derive the per-slot TEST db
#   (cubeplex_test_<slug>, matching scripts/worktree-env's `db_test_schema`) and
...
if _wt_env.exists():
    _dev_db = next(
        (
            ...
            if line.startswith("CUBEPLEX_DATABASE__NAME=")
        ),
        None,
    )
    if _dev_db and not _dev_db.startswith("cubeplex_test_"):
        # cubeplex_<slug> -> cubeplex_test_<slug>
        os.environ["CUBEPLEX_DATABASE__NAME"] = _dev_db.replace("cubeplex_", "cubeplex_test_", 1)
else:
    os.environ.setdefault("CUBEPLEX_DATABASE__NAME", "cubeplex_test")
...
_db_name = os.environ.get("CUBEPLEX_DATABASE__NAME", "cubeplex_test")
if "test" not in _db_name:
    raise RuntimeError(
        f"Refusing to run the test suite against non-test database {_db_name!r}. "
        "Unset CUBEPLEX_DATABASE__NAME (don't `source .worktree.env` before pytest) "
...
os.environ["CUBEPLEX_OBJECTSTORE__ACCESS_KEY"] = "rustfsadmin"
os.environ["CUBEPLEX_OBJECTSTORE__ACCESS_SECRET"] = "rustfsadmin"

import pytest  # noqa: E402  -- imports must follow the env setup above

from cubeplex.config import config  # noqa: E402, F401  -- initialise dynaconf after env setup
from cubeplex.plugins import (  # noqa: E402
