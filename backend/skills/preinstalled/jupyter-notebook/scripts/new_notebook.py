...
def ensure_cell_ids(notebook: dict[str, Any]) -> None:
    cells = notebook.get("cells")
    if not isinstance(cells, list):
        raise SystemExit("Template notebook cells must be a list")

    for cell in cells:
        if not isinstance(cell, dict):
            raise SystemExit("Template notebook cells must be mappings")
