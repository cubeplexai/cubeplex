...
1. Reads static OAuth client_id / client_secret env vars for connectors
   ...
   convention is ``CUBEPLEX_MCP_OAUTH__<SLUG>__CLIENT_ID`` /
...
Invocation: ``python -m cubeplex.cli seed-mcp-templates``. Not wired into
...
from cubeplex.models import Credential, MCPConnector
...
def _slug_to_env_prefix(slug: str) -> str:
    return f"CUBEPLEX_MCP_OAUTH__{slug.upper().replace('-', '_')}"
