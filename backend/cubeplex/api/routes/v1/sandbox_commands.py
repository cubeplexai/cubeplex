"""Temporary controls for pre-cutover sandbox commands without a task binding."""
...
async def list_sandbox_commands(
    ...
    """List only commands that have not yet been migrated to BackgroundTask."""
    del workspace_id
    await _require_conversation(session, ctx, conversation_id)
    repository = SandboxCommandRepository(session, org_id=ctx.org_id, workspace_id=ctx.workspace_id)
    rows = await repository.list_inflight_for_conversation(conversation_id)
    return [
        SandboxCommandOut(
            id=row.id,
...
async def kill_sandbox_command(
    ...
    """Stop only a pre-cutover command; managed tasks use the public task API."""
    del workspace_id
    await _require_conversation(session, ctx, conversation_id)
    repository = SandboxCommandRepository(session, org_id=ctx.org_id, workspace_id=ctx.workspace_id)
    row = await repository.get(command_id)
    if row is None or row.conversation_id != conversation_id or row.task_id is not None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
