...
async def claim_stale_runs(
    ...
    ``run_id`` may be absent before first dispatch or present after durable
    admission. Recovery always retains the existing binding so it can prove
    whether that exact run started; it never creates a replacement identity.
