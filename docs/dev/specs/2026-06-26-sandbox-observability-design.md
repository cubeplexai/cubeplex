...
- `_sync_skills` (`backend/cubeplex/sandbox/lazy.py`) 拿到 `Sandbox` handle → 读 PVC 上的 `/workspace/.skills/manifest.json` → diff → 增量推送 → 写回 manifest
...
新 dataclass（放在 `backend/cubeplex/sandbox/sync_result.py`）：
...
`_hash_manifest` 新 helper（放在 `backend/cubeplex/skills/sync_manifest.py`）：
...
新服务（`backend/cubeplex/sandbox/sync_events.py`）：
...
- `backend/cubeplex/sandbox/sync_result.py` —— `SyncResult` dataclass
- `backend/cubeplex/sandbox/sync_events.py` —— `UserSandboxSyncEventService`
- `backend/cubeplex/models/user_sandbox_sync_event.py` —— 新模型类（也可以并入 `models/user_sandbox.py`）
- `backend/cubeplex/api/routes/v1/admin_sandboxes.py` —— admin API 子树
- `backend/cubeplex/repositories/user_sandbox_sync_event.py` —— 仓储
...
- `backend/cubeplex/sandbox/lazy.py` —— `_sync_skills` 改返回 SyncResult；`LazySandbox._ensure_skills_synced` 加事件钩子；`LazySandbox` 加 `_user_sandbox_id` 字段；`_ensure_with_retry` 接收 `SandboxAttachment`
- `backend/cubeplex/sandbox/manager.py` —— `get_or_create` 返回类型 `Sandbox` → `SandboxAttachment`
- `backend/cubeplex/skills/sync_manifest.py` —— `_hash_manifest` 新 helper
- `backend/cubeplex/models/user_sandbox.py` —— 加 4 列
