...
strong prompt-cache stability. Weakness for Cubebox: optimized for a local
single-user agent, not multi-tenant workspaces.

Cubebox combines the useful parts and takes its own direction:
...
The adapter layer in `cubebox/llm/` takes the rendered request and decides
...
`create_cubebox_agent()` accepts memory dependencies and adds
...
- Shared memory is content other users wrote, not Cubebox instructions.
