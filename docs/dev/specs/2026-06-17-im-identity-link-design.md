...
IM users (Discord, Feishu) interact with cubeplex through a shared bot. Without
...
fails when the user's Feishu registered email differs from their cubeplex email.
...
user clicks it, logs into cubeplex, and the system verifies the email matches
before creating the `IMIdentityLink`.

## Flow

```
  IM (Discord / Feishu)                    cubeplex web
  ─────────────────────                    ───────────
  1. User sends:
  ...
  2. Bot replies (ephemeral):
     "点击链接完成绑定：
      https://app.cubeplex.com/im/link?token=eyJ..."
...
| `iss` | `"cubeplex:im-link"` (distinct from other cubeplex JWTs) |

## Verification Rules (in order)

When the user clicks the link and is authenticated:

1. **Token valid** — signature OK, not expired, `iss == "cubeplex:im-link"`.
2. **Email has account** — a `User` row exists with this email. If not:
   reject with _"该邮箱尚未注册 cubeplex，请先完成注册。"_
...
- Success: "✅ 绑定成功 — 你的 {platform} 账号已关联到此 cubeplex 账号。"
...
- No auto-creation of cubeplex accounts for unregistered IM users.
