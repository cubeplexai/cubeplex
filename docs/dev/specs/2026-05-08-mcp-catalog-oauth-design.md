...
- 环境变量 `CUBEBOX_MCP_OAUTH__<SLUG>__CLIENT_ID` / `__CLIENT_SECRET`
- `python -m cubebox.cli seed-mcp-catalog` 启动时读取并 upsert 到 `credentials` 表（`org_id=NULL`、`kind=mcp_oauth_client_secret`）
...
- state HMAC key 派生自 `CUBEBOX_AUTH__CSRF_SECRET`，redis 一次性消费
...
- `backend/cubebox/mcp/oauth/__init__.py` 头部注释
...
部署流程：`alembic upgrade head` → `python -m cubebox.cli seed-mcp-catalog`
