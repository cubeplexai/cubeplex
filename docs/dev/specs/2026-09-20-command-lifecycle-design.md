...
50. 普通用户 run 释放 slot 并正常结束后仍可提炼记忆；Stop 在提炼首个模型调用前或模型返回前发生时，不开始新调用／记忆工具。另用数据库双向 barrier 验证工具校验后至记忆提交之间的竞争：Stop 先提交则拒绝写入，记忆先持锁则先提交，不允许 Stop 提交后再写；事务回滚保留原记忆及容量清理前状态。旧批次重开、完成凭据替换或消失、会话删除均不得使旧提炼恢复写入。
