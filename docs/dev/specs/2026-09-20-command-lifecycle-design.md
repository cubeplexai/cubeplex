...
已入账费用属于组织历史，不随个人账号、会话或 workspace 删除：保留 BillingEvent／LlmBillingEvent 的原 ID、金额、币种、时间、用量和价格快照，将实际被删的 user／conversation／workspace 引用置空，仍存在的共享会话维度保留，不把旧费用记给接任者。org 报表展示稳定的“已删除用户／工作区”分组，CSV 相应 ID 为空，不另存姓名／邮箱副本；各币种总额及历史用量不变。迟到记账与删除串行，使用原组织账务来源和幂等事件 ID，已删维度不复活，记账不授予新执行权。只有组织本身获准删除才清理其账务；本次不另建通用计费系统或承诺修复所有既有崩溃丢账。
...
| EE 成本 CSV 导出 | org 及按 workspace 筛选的两个导出均维持原 org admin 资格，受同一连接租约约束；私有暂存完整文件后附长度／摘要，客户端校验完整才保存，不将多行后 EOF 当成功，不把 workspace 筛选误当普通成员授权，EE 许可边界不变 |
| 管理员 provider 测试 | 流式和非流式探测都在每次 liveness／模型结果写事务中重查原管理员资格、原 Provider／Model config_revision 及 Credential secret_revision；版本与实际探测快照绑定，编辑／轮换／回改均递增，不能用 updated_at 推测。失权或版本改变不落库，连接租约不授权写入 |
...
CSV 完整性以确切字节数和 SHA-256 校验，不以正常 EOF 或 CSV 可解析判断；受控下载只有校验成功才保存文件。请求内私有暂存与客户端读取均限制为每次 64 MiB，超限提示缩小日期范围；服务端每进程最多 4 个暂存／传输请求，忙时明确拒绝，不无限积压。取消、失权、错误和进程退出回收临时文件，无公共下载链接或持久导出任务；现有 CSV 路径保留，准备／下载／失败状态随前端及成本说明一起交付。
...
62. 删除有既有账单的用户、独占会话或 workspace 后，原 org 的各币种费用／用量／次数及历史导出保留，账单及 LLM 子行不删不重建；只有被删维度的 FK 为空，共享维度仍可用。迟到写入与删除的两个顺序、幂等重试、事务回滚和已删除维度聚合不丢金额、不复活身份，也不让其他 org 读取这些历史。
