...
owner 角色接任不等于 sandbox 已换归属。上述共享资源转移／个人 sandbox 依赖预检同时适用于 topic creator 离开、workspace／org 移除或离开：只要该操作会撤销原资源提供者的资格，就先检查所有受影响资源，不能只检查当前 role 是否 owner。尚有其他人的共享路由依赖其 user-scope 个人 sandbox 时，整个撤权请求在标记 revoked、创建回执或修改角色前明确拒绝，返回同一依赖错误；原权限不变，不宣称移除成功或清理中。先结束依赖关系并完成相关执行清理，或另行明确授权迁移后再重试。dedicated／conversation scope 可在撤权事务按同一接任规则更新资源归属，保留其他 actor 的原实例；只改 TopicParticipant.role 或把 creator 改成 B 却仍连接 A 的个人目录均不算完成。上级批量撤权也先检查全部影响范围，不得部分撤销后才发现依赖；相应成员／topic UI 必须区分未受理与 cleanup_pending。
...
54. 最后一位 topic owner 离开后，即使远端清理失败或 worker 重启，初始撤权事务已选出最早的有效接任者，后者能执行 owner 操作。并发离开／上级撤权、候选失效、已有其他 owner、无剩余成员及旧成员重新加入不产生重复晋升或权限复活。creator-mode 的个人 sandbox 依赖在 topic／workspace／org 撤权受理前检查，原 creator 即使已非 owner 也不能绕过；拒绝无部分撤权，dedicated 路径同步转移实际资源归属。
