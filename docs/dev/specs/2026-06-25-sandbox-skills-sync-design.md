...
今天的同步路径在 `backend/cubeplex/sandbox/lazy.py:26-51` 的 `_sync_skills`：
...
**单一真理源**：`backend/cubeplex/skills/sandbox_paths.py` 的 `SKILLS_ROOT` 常量是 d64daf3e 引入的唯一硬编码点，`lazy.py` 和 `load_skill` 都过 `sandbox_skill_dir(name, version)` helper。本设计只改这一处常量：
...
**Prompt 模板天然兼容**：`backend/cubeplex/prompts/skills.py` 的 `SKILLS_PROMPT_TEMPLATE` 告诉 agent 用 `load_skill` 返回的 `path` 字段 verbatim，**不硬编码路径模式**（这正是 d64daf3e 修过的 bug 的反面）。`sandbox_paths.py` 常量改了之后，`load_skill` 自动返回新路径，**prompt 不用动一个字**。
...
`backend/cubeplex/models/skill.py` 的 `SkillVersion` 加一列：
...
为让上述 `needs_push` 拿到 desired hash，`ResolvedSkill` dataclass（`backend/cubeplex/skills/service.py:33`）要加一个字段：
...
`backend/cubeplex/models/skill.py`：

```python
class SkillVersion(CubeplexBase, table=True):
    ...
    content_hash: str = Field(
        max_length=71,
        sa_column=Column(String(71), nullable=False, server_default=""),
...
- `backend/cubeplex/skills/sync_diff.py` —— `SkillSyncDiff` + `compute_skill_sync_diff`（纯函数）
- `backend/cubeplex/skills/sync_tar.py` —— `_build_tarball` + `_build_extract_and_remove_cmd`
- `backend/cubeplex/skills/content_hash.py` —— `compute_skill_version_hash` 同 / 异步两个版本
- `backend/scripts/dev/backfill_skill_version_content_hash.py`
- `backend/scripts/dev/benchmark_skill_sync.py`
...
- `backend/cubeplex/sandbox/lazy.py` —— 重写 `_sync_skills` + `LazySandbox._synced_for_this_run` flag + `_ensure_with_retry` 内嵌 sync
- `backend/cubeplex/sandbox/base.py` —— 删 `has_synced` / `mark_synced` / `_synced_skill_version_ids`
- `backend/cubeplex/models/skill.py` —— `SkillVersion.content_hash` 列
- `backend/cubeplex/skills/service.py` —— `SkillPublishService._publish_from_files` 计算 hash 并写入；`ResolvedSkill` dataclass 加 `content_hash: str` 字段；`SkillCatalogService.list_enabled_for_workspace` 把 `SkillVersion.content_hash` 透传到 ResolvedSkill 构造；`list_files_for_sandbox_sync` 签名不变（接口已合适）
- `backend/cubeplex/seeders/skill_seeder.py` —— seed 时算 hash
- `backend/cubeplex/skills/sources/*.py` —— import 时算 hash（所有 source 实现）
- `backend/cubeplex/skills/sandbox_paths.py` —— `SKILLS_ROOT` 常量从 `/.skills` 改为 `/workspace/.skills`；新 export `safe_skill_name(name)` 纯函数（拆自 `sandbox_skill_dir`）；docstring 示例同步更新
- `backend/cubeplex/sandbox/manager.py` —— 配置默认 `sandbox.volume.enabled = True`；头部注释里 `/.skills/<name>/<version>/` 改为 `/workspace/.skills/<name>/<version>/`
