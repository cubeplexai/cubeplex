...
async def _sync_skills(
    *, catalog: SkillCatalogService,
    ...
    # 1. 读 manifest（download，不用 execute）
    try:
    ...
    cmd = _build_extract_and_remove_cmd(
    ...
    new_manifest = _build_manifest(enabled)
...
- `_synced_for_this_run` 跟随 LazySandbox 实例生命周期（= 一次 cubepi run）
- 同步在 `_ensure_with_retry` 入口处串行；失败永不阻断 execute（沿用今天的兜底）
- 并发 tool call：cubepi 同 run 内的并发 execute 共享同一个 LazySandbox，首个进入触发 sync，其余被 `LazySandbox._lock` 自然串到 sync 之后
...
def needs_push(safe_name, desired, manifest) -> bool:
    ...
    if cur is None:
        return True
    return cur["version"] != desired.version or cur["content_hash"] != desired.content_hash
```

理论上 `SkillVersion` immutable，version 比较就够；hash 是「同 version 内容被覆盖」的二级防御（运维事故、redeploy 时 preinstalled 文件被替换等）。
...
```python
def _build_extract_and_remove_cmd(skills_root, has_push, to_remove) -> str:
    parts = []
    if has_push:
        parts.append(
            f"mkdir -p {shlex.quote(skills_root)} && "
            f"tar -xzf /tmp/skills_delta.tgz -C {shlex.quote(skills_root)} && "
            f"rm -f /tmp/skills_delta.tgz"
        )
    for name in to_remove:
        target = f"{skills_root}/{name}"
        parts.append(f"rm -rf {shlex.quote(target)}")
    return " && ".join(parts)
```

注意路径都用 `shlex.quote` 包裹，抗特殊字符（空格、Unicode 都安全）。
...
- sync 算法把 `""` 视为「unknown，需要重推」
- 等于「老 SkillVersion 在新 sandbox 上保守地推一次」，安全
- backfill 跑完后回到正常路径
...
- 部署上线 → seeder 写入 `skill-creator` 1.0.x 新 `SkillVersion` 行（带新 content_hash）
- 已有用户的 sandbox 在下一次 cubepi run 进入 `_sync_skills`
- manifest 里 `skill-creator.version` 是 0.2.0，desired 是 0.3.0 → 进 `to_push`
- tar.gz 推新版 → manifest 更新 → 0.2.0 目录由「下一次同 user 的 skills 集合再变化时」的 `to_remove` 路径自然回收（或第一版直接合并到本次 push 时一并清理）

零运维，无需写迁移脚本。**这条 dogfood 也是设计正确性的间接 e2e 证明**：上线后能跑通，即说明 diff / push / manifest 都对了。
...
| manifest JSON 损坏 | 同上 |
| `sandbox.upload(tar.gz)` 失败 | 抛出，被 `_sync_skills` 的 try/except 兜住；不更新 `_synced_for_this_run`；下个 run 重试 |
| `sandbox.execute(tar -xzf)` 非零退出 | 抛 `SandboxError`，manifest 未更新；下次 sync 重新算 diff，按 hash 不一致再推一遍，覆盖正确 |
| manifest upload 失败（最后一步）| 文件已就位但 manifest 旧；下次 sync 再发现"应该推的都在了"按 hash 不一致重推一次，浪费一次，不出错 |
| `rm -rf` 失败 | 残留目录；下次 sync retry。允许"最终删，时机灵活" |
| `catalog.list_enabled_for_workspace` 失败 | 沿用 `lazy.py:148` 的 try/except：log + sandbox 可用 + skills 视作不变。**不阻断 execute** |

**核心原则**：同步失败永不阻断 execute。skills 缺失的代价是 `load_skill` 返回错误，agent 看到 corrigible error 自行重试。

### 6.2 并发

- 同一 LazySandbox 内的并发 tool call：`LazySandbox._lock` 已有，sync 在锁内串行
...
- `backend/cubebox/skills/service.py` —— `SkillPublishService._publish_from_files` 计算 hash 并写入；`list_files_for_sandbox_sync` 签名不变（接口已合适）
