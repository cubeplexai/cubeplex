...
- **Date**: 2026-04-17
- **Status**: DRAFT
- **Supersedes**: `~/.gstack/projects/xfgong-cubebox/chris-main-design-20260415-192937.md`
- **Branch**: main
...
cubebox 的定位是 **Agent 基础设施层**，三个硬差异化点：
...
**受合规压力倒灌的 B2B SaaS（Series B-D, 50-500 人）**：给律所供货的 legaltech、给医院供货的 healthtech、给银行供货的 fintech、给政府的 defense-tech。他们的候选名单不是 `[cubebox, Manus, ChatGPT]`，而是 `[cubebox, 自己建一套, 继续不用 agent]`。

## 2. Product Architecture — 双产品模型

### 2.1 产品分界

| 维度 | **cubebox** (OSS) | **cubebox-admin** (企业版) |
|---|---|---|
| 定位 | 用户端：agent 执行产品 | 管理端：治理控制台 |
| 目标用户 | 团队成员（使用 agent） | 管理者/安全/合规（监管 agent） |
| 许可 | MIT / Apache 2.0 | 商业许可 |
| 仓库 | `cubebox` | `cubebox-admin`（private） |
...
**Agent 执行重构**：
14. `create_cubebox_agent()` 参数化——从 DB `AgentConfig` 读配置，不再用全局 `config.yaml`
...
### 3.2 cubebox-admin (管理端) M1 交付

**基础架构**：
1. 新 repo `cubebox-admin`，商业许可
...
- M1：从 ENV `CUBEBOX_CREDENTIAL_MASTER_KEY` 读
...
**现状**：`create_cubebox_agent()` 读全局 `config.yaml`。  
**改为**：`send_message(workspace_id)` → 查 `AgentConfig` 表 → 传给 `create_cubebox_agent(config: AgentConfig)`。
...
1. `git clone cubebox && docker-compose up`
