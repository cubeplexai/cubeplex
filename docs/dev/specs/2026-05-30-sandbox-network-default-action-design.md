...
`defaultAction`). cubebox controls evaluation order purely by the order it
...
**`"allow"`**. This is a deliberate posture choice: out of the box a sandbox
can reach the network, and an org tightens it by adding `deny` rules (or
flipping the whole policy to `deny` for a strict whitelist). The migration's
`server_default` is `'allow'`, so any pre-existing rows (created before this
column) also resolve to allow. Surfaced through `SandboxPolicyOut` /
`UpdateSandboxPolicyIn` and the admin editor.

Posture note: this weakens the previous hard-wired default-deny egress. It is
acceptable here because (a) the policy surface is admin-only and (b) the
credential-substitution boundary (vault placeholders) is independent of
network reachability, so default-allow does not expose stored secrets. An org
that wants the strict whitelist sets `network_default_action: "deny"`.
...
- Network targets remain **FQDN + `*.domain.tld` wildcard only**. Regex (the
  ...
  stays rejected — "allow all" is the default posture now
  (`network_default_action: "allow"`), expressed by the default action, never
  by a wildcard target.
...
- IP / CIDR network targets (sidecar supports them; cubebox surface is
...
| Component | Change |
|---|---|
| `backend/cubebox/models/sandbox_policy.py` | new `network_default_action` column; fix stale comment |
| `backend/alembic/versions/` | autogenerated migration (varchar, `server_default='allow'`) |
| `backend/cubebox/api/schemas/sandbox_policy.py` | add field to `SandboxPolicyOut` / `UpdateSandboxPolicyIn` |
| `backend/cubebox/services/sandbox_policy.py` | validate default action + contradictory pairs; resolver carries the field |
| `backend/cubebox/sandbox_policy/rules.py` | replace `merge_network_rules` → `build_network_policy` (specificity sort, default action, force-allow) |
| `backend/cubebox/sandbox_env/injector.py` | drop network-policy/vault-host coupling; return env + bindings only |
| `backend/cubebox/sandbox/manager.py` | assemble policy from admin rules + default action + forced exchange host |
| `backend/cubebox/api/routes/v1/admin_sandbox_policy.py` | pass the new field through |
