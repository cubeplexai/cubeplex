...
`"deny"` (existing rows keep current behavior). Surfaced through
`SandboxPolicyOut` / `UpdateSandboxPolicyIn` and the admin editor.
...
| `backend/alembic/versions/` | autogenerated migration (varchar, default `'deny'`) |
