...
The mental model: cubebox already has a complete attachment pipeline for
...
(`cubebox/streams/run_manager.py:867`) already accepts attachment ids and:
...
content: bytes, mime_type)` (`cubebox/services/attachments.py:177`). It
...
`_load_secrets` / `_client_for` closures in `cubebox/im/runtime.py:134-168`
...
`InboundEvent` (`cubebox/im/types.py:114`) gains
...
`IMRunQueueItem` (`cubebox/models/im_connector.py:142`) gains **two** nullable
...
Today (`cubebox/im/artifacts.py:40`): an `image` artifact on Feishu
...
`cubebox/prompts/artifacts.py:12-21` — `website` / `document` / `image` /
...
`OutboundRunTailer.run()` (`cubebox/im/outbound.py:531+`) — after the terminal
...
`cubebox/im/types.py`) covering **every method the artifact dispatcher calls on
...
New **bound-connector** Protocol `OutboundConnector` (`cubebox/im/types.py`),
