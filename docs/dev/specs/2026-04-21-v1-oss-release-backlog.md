...
> **⚠️ 本节已被取代（2026-07-07 / 2026-07-27）。** 三条边界决策后来反转了：EE 改为
> **同仓 `ee/` 目录 + 源码公开**（非独立仓、非闭源），发包收敛为**单个 `cubeplex-ee`
> wheel**，插件发现从 `entry_points` 扫描改为**单点可选 import**。当前口径见
> [`2026-07-07-oss-ee-split-design.md`](2026-07-07-oss-ee-split-design.md)。
> 下面几条按原样保留，作为当时的记录。
