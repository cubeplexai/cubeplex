# Cubebox Agent 记忆机制梳理

> 2026-06-02 — 基于 cubebox backend + cubepi 源码探索整理

---

## 整体架构

cubebox 的"记忆"实际上由三层组成，承担不同生命周期的职责：
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `thread_id` | PK | 对应 cubebox 的 `conversation_id` |
...
**`MemoryItem`** (`cubebox/models/memory.py`)
...
文件：`cubebox/middleware/memory.py`
...
文件：`cubebox/middleware/compaction/__init__.py`
...
`cubebox/streams/run_manager.py` 中组装，`transform_context` / `transform_system_prompt` 按顺序调用：
...
| `cubebox/models/memory.py` | MemoryItem 模型（scope / type / status） |
| `cubebox/middleware/memory.py` | MemoryMiddleware，pinned/relevance 两 tier，snapshot 计算 |
| `cubebox/middleware/compaction/__init__.py` | CompactionMiddleware，摘要生成与压缩视图 |
| `cubebox/services/memory_consolidation.py` | 后台 consolidation 门控与执行 |
| `cubebox/streams/run_manager.py` | middleware stack 组装，snapshot 捕获，UserMessage 构建 |
| `cubebox/agents/graph.py` | create_cubebox_agent factory |
