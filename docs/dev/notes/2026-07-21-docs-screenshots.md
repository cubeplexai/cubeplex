...
- After PR review flagged test-environment details in two existing captures,
  temporarily replaced the model-provider endpoints with public demo URLs and
  the sandbox policy image with `cubeplex/sandbox:24.04` in the remote test
  database, then recaptured `admin/models-providers.png` and
  `admin/sandbox-network-policy.png`. The original test configuration was
  restored after capture; the refreshed assets contain no private LAN hosts,
  internal gateways, or registry names.
