...
- 工作目录：`/home/chris/cubeplex/.worktrees/feat/2026-06-25-sandbox-skills-sync`
...
| 路径 | 责任 |
|---|---|
| `backend/cubeplex/skills/content_hash.py` | sha256 内容哈希函数（同步 + 异步 wrapper） |
| `backend/cubeplex/skills/sync_diff.py` | `SkillSyncDiff` dataclass + `compute_skill_sync_diff` 纯函数 |
| `backend/cubeplex/skills/sync_tar.py` | tar.gz 打包 + sandbox 端 extract/rm shell 命令构建 |
| `backend/cubeplex/skills/sync_manifest.py` | manifest schema 序列化 + `_build_manifest` |
...
| 路径 | 修改要点 |
|---|---|
| `backend/cubeplex/models/skill.py` | `SkillVersion` 加 `content_hash: str` 列 |
| `backend/cubeplex/skills/sandbox_paths.py` | `SKILLS_ROOT` 改值；export 新 helper `safe_skill_name`；docstring 更新 |
| `backend/cubeplex/skills/service.py` | `SkillPublishService._publish_from_files` 算 hash 写入；`ResolvedSkill` 加 `content_hash: str` 字段；`SkillCatalogService.list_enabled_for_workspace` 把 `SkillVersion.content_hash` 透传到 ResolvedSkill 构造（F1） |
| `backend/cubeplex/seeders/skill_seeder.py` | seed 时算 hash 写入 |
| `backend/cubeplex/skills/sources/clawhub.py` 等 | import 时算 hash（依据 sources 目录下实际文件枚举） |
| `backend/cubeplex/sandbox/base.py` | 删 `has_synced` / `mark_synced` / `_synced_skill_version_ids` |
| `backend/cubeplex/sandbox/lazy.py` | 重写 `_sync_skills`；`LazySandbox` 加 `_synced_for_this_run` |
| `backend/cubeplex/sandbox/manager.py` | 头部注释路径更新；`sandbox.volume.enabled` 配置默认 True |
| `backend/skills/preinstalled/skill-creator/SKILL.md` | 4 处 `/.skills/` 路径改 `/workspace/.skills/`；frontmatter version `0.2.0` → `0.3.0` |
| `backend/cubeplex/config.py` 或对应配置文件 | `sandbox.volume.enabled` 默认值 |
...
**Files:**
- Modify: `backend/cubeplex/models/skill.py:49-64`
...
打开 `backend/cubeplex/models/skill.py`，找到 `class SkillVersion(CubeplexBase, table=True):` 块（line 49-64），在 `uploaded_by_user_id` 字段后、`__table_args__` 之前插入：
...
```bash
cd backend && uv run mypy cubeplex/models/skill.py 2>&1 | tee ../tmp/task-1.1-mypy.log | tail -5
```
...
**Files:**
- Create: `backend/cubeplex/skills/content_hash.py`
...
期望：`ModuleNotFoundError: No module named 'cubeplex.skills.content_hash'`。

- [ ] **Step 3: 实现 content_hash 模块**

创建 `backend/cubeplex/skills/content_hash.py`：
...
```bash
cd backend && uv run mypy cubeplex/skills/content_hash.py 2>&1 | tail -3
```
...
```bash
git add backend/cubeplex/skills/content_hash.py backend/tests/unit/test_skill_content_hash.py
...
**Files:**
- Modify: `backend/cubeplex/skills/service.py` (查找 `_publish_from_files`)
...
```bash
grep -n "_publish_from_files\|SkillVersion(" backend/cubeplex/skills/service.py | head -10
```
...
```bash
git add backend/cubeplex/skills/service.py backend/tests/e2e/test_skill_publish_content_hash_e2e.py
...
**Files:**
- Modify: `backend/cubeplex/seeders/skill_seeder.py`
...
```bash
grep -n "SkillVersion(\|files\[" backend/cubeplex/seeders/skill_seeder.py | head -10
```
...
async def test_seeder_writes_content_hash(session):
    from cubeplex.seeders.skill_seeder import _install_preinstalled_skills_safe
    # ...
...
```bash
git add backend/cubeplex/seeders/skill_seeder.py backend/tests/e2e/test_skills_seeder.py
...
**Files:**
- Modify: `backend/cubeplex/skills/sources/*.py`（针对所有有写 SkillVersion 的 source adapter）
...
```bash
ls backend/cubeplex/skills/sources/
grep -rn "SkillVersion(" backend/cubeplex/skills/sources/ | head -20
```
...
```bash
git add backend/cubeplex/skills/sources/
...
**Files:**
- Modify: `backend/cubeplex/skills/service.py:33-67`（`ResolvedSkill` dataclass + `list_enabled_for_workspace`）
...
打开 `backend/cubeplex/skills/service.py`，找到第 33 行的 `@dataclass(frozen=True) class ResolvedSkill:` 块，在最后一个字段后追加：
...
```bash
cd backend && uv run mypy cubeplex/skills/service.py 2>&1 | tail -3
```
...
async def main() -> None:
    from cubeplex.config import get_settings   # project settings expose skill_cache_root
    ...
    async with AsyncSession(engine) as session:
        ...
        # skill_cache_root 字段或者 tmp 子目录）。如果不知道，看 backend/cubeplex/
...
**注意**：`SkillCache` 真实签名是 `SkillCache(cache_root: Path)`（`backend/cubeplex/skills/cache.py:20`），没有 `from_env` 工厂方法。脚本里要从 settings 取 cache root 然后显式 `SkillCache(Path(...))`。如果 settings 路径名跟例子里的 `settings.skills.cache_root` 不一样，搜 `SkillCache(` 看 `backend/cubeplex/streams/run_manager.py` 怎么实例化的，照搬。
...
```bash
cd backend && uv run mypy cubeplex/ 2>&1 | tee ../tmp/pr1-mypy.log | tail -3
```
...
**Files:**
- Modify: `backend/cubeplex/skills/sandbox_paths.py`
...
Both the sandbox file-sync (``cubeplex.sandbox.lazy._sync_skills``) and the
...
```bash
grep -rn "SKILLS_ROOT\|sandbox_skill_dir\|safe_skill_name" backend/cubeplex/ backend/tests/ 2>&1 | grep -v __pycache__
```
...
```bash
cd backend && uv run mypy cubeplex/skills/sandbox_paths.py 2>&1 | tail -3
...
```bash
git add backend/cubeplex/skills/sandbox_paths.py backend/tests/unit/test_sandbox_paths.py
...
**Files:**
- Modify: `backend/cubeplex/sandbox/manager.py:8-20`

**Interfaces:** 注释更新；无代码行为变化

- [ ] **Step 1: 改头部注释**

`backend/cubeplex/sandbox/manager.py` 顶部模块注释里有：
...
**Files:**
- Create: `backend/cubeplex/skills/sync_diff.py`
...
from cubeplex.skills.sync_diff import SkillSyncDiff, compute_skill_sync_diff
...
期望：`ModuleNotFoundError: No module named 'cubeplex.skills.sync_diff'`。

- [ ] **Step 3: 写实现**

创建 `backend/cubeplex/skills/sync_diff.py`：
...
```bash
cd backend && uv run mypy cubeplex/skills/sync_diff.py 2>&1 | tail -3
```
...
```bash
git add backend/cubeplex/skills/sync_diff.py backend/tests/unit/test_skill_sync_diff.py
...
**Files:**
- Create: `backend/cubeplex/skills/sync_tar.py`
...
期望：`ModuleNotFoundError: No module named 'cubeplex.skills.sync_tar'`。

- [ ] **Step 3: 写实现**

创建 `backend/cubeplex/skills/sync_tar.py`：
...
```bash
git add backend/cubeplex/skills/sync_tar.py backend/tests/unit/test_skill_sync_tar.py
...
**Files:**
- Create: `backend/cubeplex/skills/sync_manifest.py`
...
创建 `backend/cubeplex/skills/sync_manifest.py`：
...
```bash
git add backend/cubeplex/skills/sync_manifest.py backend/tests/unit/test_skill_sync_manifest.py
...
**Files:**
- Modify: `backend/cubeplex/sandbox/lazy.py` 整个 `_sync_skills` 函数
...
打开 `backend/cubeplex/sandbox/lazy.py`，找到第 19-51 行的 `import` 和旧 `_sync_skills`。替换为：

```python
from cubeplex.sandbox.base import SandboxError
from cubeplex.skills.sandbox_paths import SKILLS_ROOT, safe_skill_name
from cubeplex.skills.sync_diff import compute_skill_sync_diff
from cubeplex.skills.sync_manifest import MANIFEST_PATH, build_manifest, parse_manifest
from cubeplex.skills.sync_tar import build_extract_and_remove_cmd, build_tarball
...
确认旧 `sandbox_skill_dir` import 已被新 import 块取代（lazy.py 旧第 19 行的 `from cubeplex.skills.sandbox_paths import sandbox_skill_dir` 删掉）。
...
**Files:**
- Modify: `backend/cubeplex/sandbox/base.py:175-190`
...
打开 `backend/cubeplex/sandbox/base.py`，找到第 175-190 行的 `has_synced` / `mark_synced` 方法块，**整段删除**。

- [ ] **Step 2: grep 确认无 stale 调用方**

```bash
grep -rn "has_synced\|mark_synced\|_synced_skill_version_ids" backend/cubeplex/ backend/tests/ 2>&1 | grep -v __pycache__
```
...
```bash
cd backend && uv run mypy cubeplex/ 2>&1 | tee ../tmp/task-2.8-mypy.log | tail -3
```
...
```bash
git add backend/cubeplex/sandbox/lazy.py backend/tests/unit/test_lazy_sandbox_sync_lifecycle.py
...
from cubeplex.models import Workspace, Organization
from cubeplex.sandbox.lazy import LazySandbox
...
如果项目实际的 install 路径走 `SkillInstallService` / `SkillRegistry` 而不是 OrgSkillInstall 直插，查 `backend/cubeplex/skills/service.py` 看 `_publish_from_files` 之后 install 逻辑在哪，照搬。
...
```bash
grep -rn "volume\.enabled\|volume_enabled\|VolumeConfig\|class SandboxVolume" backend/cubeplex/ 2>&1 | grep -v __pycache__ | head -20
```
...
```bash
ls backend/config/ backend/cubeplex/config.py 2>&1 | head -10
...
**Files:**
- Modify: `backend/cubeplex/config.py` 或 `backend/cubeplex/sandbox/manager.py:149` 周围的 SandboxConfig dataclass / pydantic Settings 类
...
```bash
git add backend/cubeplex/config.py backend/config/*.yaml backend/cubeplex/sandbox/manager.py
