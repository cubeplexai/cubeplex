...
- 状态：实施中，遵循已确认的 [spec](../specs/2026-09-20-command-lifecycle-design.md) 和已收缩的生命周期范围。#635 已补齐 C1 的一次性 monitor 与公共结果就绪契约；#636 已接通明确目标的停止控制、保护重复 Stop 下的收尾，并补齐持久等待问题中的停止恢复。上述增量已通过回归及本地 Codex 复审；preparing、问题已清除后的中断和终态收尾恢复仍未完成，不代表整个 C2 已完成。新 coordinator 仍未启用，部署与线上数据操作需独立授权。
...
| 本轮已补齐的恢复 | 持久等待问题中的 Stop／全部停止可在 worker 重启、Redis 暂停记录过期及清理租约到期后继续清理；不重新调用模型，不要求原 actor 恢复权限 | C2a，仅暂停分支 |
| 下一步接入 | preparing、等待问题已清除后的中断、终态回执／占用释放失败的持久恢复；随后接通真实输入来源写入和前端调用 | C2a，之后 C2b、C5 |
...
- 以上证明 C2a 的持久控制、前台任务清理、停止 HTTP 和持久暂停恢复增量。preparing、问题已清除／终态收尾的恢复、实际输入来源写入、前端稳定目标调用仍需后续提交和独立证据；C2b–C2d、C3–C7、R 的宿主集成仍未完成。未修改用户确认的 spec 范围，未触发 GitHub Codex 审核。
