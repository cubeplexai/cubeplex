...
- 状态：实施中，遵循已确认的 [spec](../specs/2026-09-20-command-lifecycle-design.md) 和已收缩的生命周期范围。#635 已补齐 C1 的一次性 monitor 与公共结果就绪契约；#636 已拆开 run Stop／批次关闭的持久权限、接通明确目标的停止 HTTP 接口，并修复取消后重复补交接及重复 Stop 打断收尾。上述增量已通过回归及本地 Codex 复审；C2a 的持久恢复仍未完成，不代表整个 C2 已完成。新 coordinator 仍未启用，部署与线上数据操作需独立授权。
...
| 下一步接入 | preparing／HITL／终态收尾失败的持久恢复；随后接通真实输入来源写入和前端调用 | C2a，之后 C2b、C5 |
...
- 以上证明 C2a 的持久控制、前台任务清理及停止 HTTP 增量。run／HITL 重启恢复、实际输入来源写入、前端稳定目标调用仍需后续提交和独立证据；C2b–C2d、C3–C7、R 的宿主集成仍未完成。未修改用户确认的 spec 范围，未触发 GitHub Codex 审核。
