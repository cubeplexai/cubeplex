...
- 状态：实施中，遵循已确认的 [spec](../specs/2026-09-20-command-lifecycle-design.md) 和已收缩的生命周期范围。#635 已补齐 C1 的一次性 monitor 与公共结果就绪契约，通过定向回归及本地 Codex 复审；聊天 Stop 的新边界和后续接入尚未完成。新 coordinator 仍未启用，部署与线上数据操作需独立授权。
...
- 按用户要求使用 local Codex review，不再触发 GitHub Codex 审核。每轮先独立核实发现的问题，修复本单元范围内的问题并复审；审核结论与测试证据直接向用户说明，不借审核扩大范围。
...
| 下一步调整 | #636 的批次关闭拆出独立 run Stop；同步 C1 更新后，验证停止与前后台交接、恢复的边界 | C2a |
...
下一步进入 C2a，再按 C2b–C2d 接入。C2 的既有记忆／收尾保护列在[保留回归](#c2-regressions)，不重新包装成一组新增功能。C1 的通过只覆盖持久运行时；C3/C4 消费公共结果契约、C6 日志确认联调及 C7 切换仍须分别验证。
...
本次实施不重开此前移除的通用删除协调与跨系统治理。#635 的旧 monitor 限流仅保留为历史记录；新一次性契约的 C1 证据如上。C2 仍需拆出 run 停止与批次关闭，C3–C5 接通交接、通知和 UI，C6/C7 完成日志联调与切换门槛后才能启用。沿用既有原实例与记忆事务保护，不为这两项产品变化扩展其他子系统。
