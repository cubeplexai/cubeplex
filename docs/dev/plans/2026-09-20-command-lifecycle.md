...
以下分类依据文末保存的实施记录；本次整理没有重新核验远端 PR 状态，也不把历史测试当作新契约验收。

| 类别 | 已有基础／剩余工作 | 对应单元 |
| --- | --- | --- |
| 可复用，保留回归 | #634 的基础模型、reservation 与默认期限；#635 的原实例／owner／停止事实；#636 的受理快照、run 回执和记忆事务保护 | C1、C2 |
| 需按新设计调整 | #635 的多次 monitor 通知改为一次最终结果；#636 的批次关闭拆出独立 run Stop；先确定结果就绪契约 | C1、C2a |
...
下一步先完成 C1 的一次性 monitor 和公共结果契约，再按 C2 小节接入。C2 的既有记忆／收尾保护列在[保留回归](#c2-regressions)，不重新包装成一组新增功能。所有代码均须通过新契约回归后才能进入 C7。
...
以下保留 2026-09-21 的核对记录，仅说明当时的提交与验证；不代表当前远端状态，也不代表新版契约已经实现。本次只重组 plan，没有重新运行这些测试或触发 GitHub Codex 审核。
