...
The agent runs code in the sandbox through shell-based tools — it issues commands, writes files, and runs scripts (for example, `python script.py`). It is not tied to a fixed list of languages. Command output streams into the chat while the command is still running. Commands that take more than 15 seconds continue in the background (except a bare `sleep`); servers started with `notify_on_complete=false` stay attached to the conversation after the turn. The sandbox panel lists running jobs and lets you stop them. Use `monitor` for log-line predicates, not builds.
