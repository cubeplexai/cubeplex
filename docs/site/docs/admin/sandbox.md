...
The agent runs code in the sandbox through shell-based tools — it issues commands, writes files, and runs scripts (for example, `python script.py`). It is not tied to a fixed list of languages. Command output streams into the chat while the command is still running. Commands that take more than 60 seconds become durable background tasks (except a bare `sleep`), allowing the current reply to finish. Execute tasks use the configured command deadline (one hour by default), even when `notify_on_complete=false`. A monitor script checks one condition and emits one final success, failure, or timeout event; its output lines are logs, not repeated wake-ups. The sandbox panel lists running jobs and lets you request that one stop.
