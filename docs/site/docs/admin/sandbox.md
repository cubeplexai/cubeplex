...
The agent runs code in the sandbox through a single shell-based `execute` tool — it issues shell commands, writes files via heredocs, and runs scripts (for example, `python script.py`). It is not tied to a fixed list of languages. Command output streams into the chat while the command is still running. Long jobs can be started with `background=true`; the agent is notified when they finish, and `kill_execute` stops them.
