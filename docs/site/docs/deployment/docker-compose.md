...
frontend share one app version, e.g. `v0.8.0` — and set it as `BACKEND_TAG` /
...
BACKEND_TAG=v0.8.0        # a release version from the releases page
FRONTEND_TAG=v0.8.0
...
The same applies to an active command whose original sandbox instance or
provider process handle was not recorded: resolve or stop it before retrying.
The migrator does not guess that it ended and does not attach it to a
replacement sandbox.
