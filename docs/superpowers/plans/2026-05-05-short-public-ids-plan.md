...
**Architecture:** New module `cubebox.models.public_id` provides one function `generate_public_id(prefix)` that returns `{prefix}-{14 base62 chars}` where the body packs a 41-bit ms-since-2024 timestamp + 42-bit randomness with an in-process monotonic factory. Every model swaps its `default_factory` from `uuid7` to `generate_public_id(<prefix>)` and shrinks `max_length` to 20. All FK columns are made explicit. Alembic revisions are wiped and one fresh baseline replaces them.
...
**Working directory:** `/home/chris/cubebox/.worktrees/feat/short-public-ids`. All commands assume `cd backend` from that root unless otherwise noted. Use `uv run` for Python commands.
...
**Create:**
- `backend/cubebox/models/public_id.py` — generator + prefix constants
...
- `backend/cubebox/models/__init__.py` (no change expected, but verify import is still clean after edits)
- All 19 model files in `backend/cubebox/models/`:
...
Expected: ImportError / ModuleNotFoundError (`cubebox.models.public_id` does not yet exist).
...
**Files:**
- Create: `backend/cubebox/models/public_id.py`
...
**Files:**
- Modify: `backend/cubebox/models/mixins.py`

- [ ] **Step 3.1: Replace mixin contents**

Open `backend/cubebox/models/mixins.py` and replace the `OrgScopedMixin` class. The full file should read:
...
```bash
git add cubebox/models/mixins.py
...
- Modify: `backend/cubebox/models/workspace.py`
...
```bash
cd backend
uv run python -c "from cubebox.models import Organization, Workspace, User; print('ok')"
```
...
```bash
git add cubebox/models/organization.py cubebox/models/workspace.py cubebox/models/user.py
...
**Files:**
- Modify: `backend/cubebox/models/membership.py`
- Modify: `backend/cubebox/models/invite_token.py`
...
```bash
git add cubebox/models/membership.py cubebox/models/invite_token.py
...
- Modify: `backend/cubebox/models/attachment.py`
...
```bash
git add cubebox/models/conversation.py cubebox/models/attachment.py \
        cubebox/models/artifact.py cubebox/models/artifact_version.py
...
**Files:**
- Modify: `backend/cubebox/models/agent_config.py`
...
```bash
git add cubebox/models/agent_config.py cubebox/models/user_sandbox.py
...
- Modify: `backend/cubebox/models/org_provider_override.py`
...
```bash
cd backend
grep -rn "OrgSettings" cubebox/ tests/
```
...
```bash
git add cubebox/models/org_settings.py cubebox/models/org_provider_override.py
...
- Modify: `backend/cubebox/models/credential.py`
...
```bash
git add cubebox/models/provider.py cubebox/models/credential.py
...
**Files:**
- Modify: `backend/cubebox/models/billing.py`
...
```bash
git add cubebox/models/billing.py
...
```bash
cd backend
grep -rn "WorkspaceMCPBinding" cubebox/ tests/
```
...
```bash
cd backend
grep -rn "WorkspaceSkillBinding\.\|OrgPreinstalledTombstone\." cubebox/ tests/
```
...
```bash
git add cubebox/models/skill.py
...
```bash
cd backend
uv run python -c "from cubebox.models import *; print('ok')"
```
...
```bash
cd /home/chris/cubebox/.worktrees/feat/short-public-ids
cat .worktree.env
```

Note `CUBEBOX_DATABASE__NAME` (e.g. `cubebox_feat_short_public_ids`). Drop and recreate it:

```bash
DBNAME=$(grep '^CUBEBOX_DATABASE__NAME=' .worktree.env | cut -d= -f2)
...
```bash
cd /home/chris/cubebox/.worktrees/feat/short-public-ids/backend
...
cd backend
uv run python -c "
from cubebox.db import engine  # or wherever the engine factory lives
...
If `cubebox.db` isn't the right module path (engine factory may live elsewhere), discover it:

```bash
cd backend
grep -rn "create_engine\|sessionmaker\|create_async_engine" cubebox/ | head
```
...
```bash
cd /home/chris/cubebox/.worktrees/feat/short-public-ids
TESTDB=$(grep '^CUBEBOX_DATABASE__NAME=' .worktree.env | cut -d= -f2)
TESTDB="${TESTDB/cubebox_/cubebox_test_}"
psql -h localhost -U postgres -c "DROP DATABASE IF EXISTS ${TESTDB};"
psql -h localhost -U postgres -c "CREATE DATABASE ${TESTDB};"
```

(The provisioning script created `cubebox_test_feat_short_public_ids` alongside the dev DB; verify the name with `\l` in psql if unsure.)
...
```bash
cd /home/chris/cubebox/.worktrees/feat/short-public-ids/frontend
pnpm install
pnpm --filter @cubebox/core build
```

Expected: clean build. Type errors here would mean `@cubebox/core` had hardcoded ID assumptions — none expected, but if found, fix and rebuild.
