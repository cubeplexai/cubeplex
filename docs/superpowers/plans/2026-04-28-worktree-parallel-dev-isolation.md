...
| `backend/cubebox/config.py` | Add 5-line dotenv preamble | Modify (line 26) |
...
- Writes `/home/chris/cubebox/.worktree.env` with `PORT=3000`, `CUBEBOX_API__PORT=8000`, `CUBEBOX_DATABASE__NAME=cubebox_main`, etc.
...
```bash
rm /home/chris/cubebox/.worktree.env
```
...
- Writes `.worktree.env`
- Loads it (sets `CUBEBOX_DATABASE__NAME=cubebox_main` in the script's env)
- Creates `cubebox_main` and `cubebox_test_main` schemas in MySQL
- Runs `alembic upgrade head` against `cubebox_main` — should succeed and apply all migrations
- Prints final `show` summary

Verify the schemas exist:

```bash
mysql -h "$(grep CUBEBOX_DATABASE__HOST backend/.env | cut -d= -f2)" \
      -u "$(grep CUBEBOX_DATABASE__USER backend/.env | cut -d= -f2)" \
      -p"$(grep CUBEBOX_DATABASE__PASSWORD backend/.env | cut -d= -f2)" \
      -P "$(grep CUBEBOX_DATABASE__PORT backend/.env | cut -d= -f2)" \
      -e "SHOW DATABASES LIKE 'cubebox_%main%';"
```

Expected: lists `cubebox_main` and `cubebox_test_main`.
...
def _redis_delete_prefix(prefix: str) -> int:
    ...
    """
    redis_url = os.environ.get("CUBEBOX_REDIS__URL") or _read_redis_url_from_env_file()
    if not redis_url:
        print("⚠ no CUBEBOX_REDIS__URL set; skipping Redis cleanup", file=sys.stderr)
        return 0
...
def _read_redis_url_from_env_file() -> str | None:
    """Fallback: read CUBEBOX_REDIS__URL from backend/.env if not in os.environ."""
    info = current_worktree_info()
    env_file = info.path / "backend" / ".env"
    if not env_file.exists():
        return None
    for line in env_file.read_text().splitlines():
        if line.startswith("CUBEBOX_REDIS__URL="):
...
def cmd_doctor(args) -> int:
    ...
    # 4. Redis reachable
    redis_url = os.environ.get("CUBEBOX_REDIS__URL")
    if redis_url:
        result = subprocess.run(
        ...
        issues.append("CUBEBOX_REDIS__URL not set")
...
def cmd_clean_orphans(args) -> int:
    ...
    # 3. Orphan MySQL schemas (cubebox_* not referenced by any live entry)
    ...
    referenced = referenced_dev | referenced_test | {"cubebox", "cubebox_test"}
...
In `backend/cubebox/config.py`, find lines 26–31:
...
```bash
cd backend && uv run python -c "from cubebox.config import config; print(config.api.port, config.database.name, config.redis.key_prefix)"
```

Expected (depends on whether you have a `.worktree.env` in repo root from earlier smoke tests):
- If `.worktree.env` exists with `CUBEBOX_API__PORT=8000 CUBEBOX_DATABASE__NAME=cubebox_main CUBEBOX_REDIS__KEY_PREFIX=cubebox-main`: prints `8000 cubebox_main cubebox-main`.
- If no `.worktree.env`: prints `8000 cubebox cubebox` (defaults).
...
```bash
git add backend/cubebox/config.py
...
- `.worktree.env` written with `PORT=300K`, `CUBEBOX_API__PORT=800K`
- Schemas `cubebox_test_wt_isolation_a` and `cubebox_test_test_wt_isolation_a` created
...
Expected: both pass without interfering. (Before this design, running both at once would fail intermittently due to shared `cubebox_test` schema and `FLUSHDB`.)
