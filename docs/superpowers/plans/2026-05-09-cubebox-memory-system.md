# Cubebox Memory System Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship the v1 memory system from `docs/superpowers/specs/2026-05-08-cubebox-memory-system-design.md` — three-scope memory (personal/workspace/org), agent tools, REST API, Memory Center UI, and the prompt-cache snapshot channel that keeps cost in check.
...
- `docs/superpowers/specs/2026-05-08-cubebox-memory-system-design.md` — full design.
- `backend/CLAUDE.md` "Prompt Cache Discipline" section — the rules this plan operationalizes.
- `backend/cubebox/agents/graph.py` — how `create_cubebox_agent()` wires middleware.
- `backend/cubebox/middleware/timestamps.py` and `middleware/skills.py` — minimal middleware shape.
- `backend/cubebox/middleware/_utils.py` — `append_to_system_message` helper.
- `backend/cubebox/models/artifact.py` — model + `OrgScopedMixin` pattern.
- `backend/cubebox/models/credential.py` — pattern for tables that mix org-scoped and "system" (NULL org_id) rows. Memory follows the same shape with three NULL combinations.
- `backend/cubebox/repositories/base.py` — `ScopedRepository`. Memory needs a **custom** repository because rows can have NULL `org_id`/`workspace_id`; do not subclass `ScopedRepository`.
...
│   ├── agents/
│   │   ├── state.py                                       [new] CubeboxState TypedDict + reducers
...
- [ ] **Step 1: Open `backend/cubebox/models/public_id.py` and locate the existing `PREFIX_*` constant block (search for `PREFIX_`).**
...
**Files:**
- Create: `backend/cubebox/models/memory.py`
...
- [ ] **Step 1: Create `backend/cubebox/models/memory.py`:**
...
- [ ] **Step 2: Register the model so Alembic can pick it up. Open `backend/cubebox/models/__init__.py` and add an export:**

```python
from cubebox.models.memory import MemoryItem, MemoryScope, MemoryType, MemoryStatus, MemorySourceType  # noqa: F401
```

- [ ] **Step 3: Type-check.**

```bash
cd backend && uv run mypy cubebox/models/memory.py
```
...
```bash
git add backend/cubebox/models/memory.py backend/cubebox/models/__init__.py
...
**Files:**
- Create: `backend/cubebox/repositories/memory.py`
...
- [ ] **Step 1: Create `backend/cubebox/repositories/memory.py`:**
...
```bash
cd backend && uv run mypy cubebox/repositories/memory.py
```
...
```bash
git add backend/cubebox/repositories/memory.py
...
**Files:**
- Create: `backend/cubebox/services/memory.py`

- [ ] **Step 1: Create `backend/cubebox/services/memory.py`:**
...
```bash
cd backend && uv run mypy cubebox/services/memory.py cubebox/services/memory_screen.py
```
...
```bash
git add backend/cubebox/services/memory.py backend/cubebox/services/memory_screen.py
...
**Files:**
- Create: `backend/cubebox/api/routes/v1/memory.py`
- Modify: `backend/cubebox/api/routes/__init__.py` (or wherever v1 routers register)

- [ ] **Step 1: Inspect an existing v1 route for shape — e.g., `backend/cubebox/api/routes/v1/artifacts.py` — and follow the same `request_context` dependency, scoping, and error patterns.**

- [ ] **Step 2: Create `backend/cubebox/api/routes/v1/memory.py`:**
...
from cubebox.api.deps import RequestContext, get_db_session, request_context
from cubebox.models.memory import (
    MemoryScope,
    MemoryStatus,
    MemoryType,
...
- [ ] **Step 3: Register the router. Find the file that mounts v1 routers (likely `backend/cubebox/api/__init__.py` or `cubebox/api/routes/__init__.py`) and add:**
...
```bash
cd backend && uv run mypy cubebox/api/routes/v1/memory.py && uv run ruff check cubebox/api/routes/v1/memory.py
```
...
```bash
git add backend/cubebox/api/routes/v1/memory.py backend/cubebox/api/
...
### Task 3.1: `CubeboxState` with `memory_snapshots` channel
...
- [ ] **Step 1: Create `backend/cubebox/agents/state.py`:**
...
Cubebox adds `memory_snapshots`: per-user-message immutable captures of the
...
class CubeboxState(AgentState):
    """Cubebox extends the default AgentState with memory_snapshots."""
...
```bash
cd backend && uv run mypy cubebox/agents/state.py
```
...
git commit -m "feat(memory): CubeboxState with immutable memory_snapshots channel"
```

---

### Task 3.2: Wire `state_schema=CubeboxState` into the agent factory

**Files:**
- Modify: `backend/cubebox/agents/graph.py`

- [ ] **Step 1: In `backend/cubebox/agents/graph.py`, add the import near the top:**

```python
from cubebox.agents.state import CubeboxState
```

- [ ] **Step 2: Locate the `create_agent(...)` call inside `create_cubebox_agent`. Add `state_schema=CubeboxState` to the kwargs. Example:**
...
graph = create_agent(
    ...
    state_schema=CubeboxState,           # <-- add this
...
git commit -m "feat(memory): use CubeboxState in agent factory"
```
...
**Files:**
- Create: `backend/cubebox/llm/cache_markers.py`
...
- [ ] **Step 1: Create `backend/cubebox/llm/cache_markers.py`:**
...
```bash
cd backend && uv run mypy cubebox/llm/cache_markers.py
```
...
```bash
git add backend/cubebox/llm/cache_markers.py backend/tests/test_cache_markers.py
...
**Files:**
- Create: `backend/cubebox/prompts/memory.py`

- [ ] **Step 1: Create `backend/cubebox/prompts/memory.py`:**
...
```bash
cd backend && uv run mypy cubebox/prompts/memory.py
```
...
```bash
git add backend/cubebox/prompts/memory.py
...
```bash
cd backend && uv run mypy cubebox/middleware/memory.py
```
...
```bash
git add backend/cubebox/middleware/memory.py
git commit -m "feat(memory): MemoryMiddleware with pinned/relevance/snapshot"
```

---

### Task 4.3: Wire `MemoryMiddleware` into `create_cubebox_agent`

**Files:**
- Modify: `backend/cubebox/agents/graph.py`
- Modify: `backend/cubebox/agents/runtime.py` (or wherever `create_cubebox_agent` is called from API routes; probably in conversation/run service)

- [ ] **Step 1: In `backend/cubebox/agents/graph.py`, add a `memory_repo_factory` parameter to `create_cubebox_agent`:**
...
- [ ] **Step 3: Update the call site that constructs the agent (search for `create_cubebox_agent(`). Pass a callable that builds a `MemoryRepository` for the current request:**
...
```bash
git add backend/cubebox/agents/graph.py [other modified files]
...
**Files:**
- Modify: `backend/cubebox/llm/factory.py` (or wherever the `BaseChatModel` is built / invoked)
...
- [ ] **Step 1: Inspect `backend/cubebox/llm/factory.py` to find where the model object is created and where the provider id is known.**
...
```python
# backend/cubebox/llm/factory.py (additions)
from cubebox.llm.cache_markers import apply_cache_markers, detect_provider
...
```bash
cd backend && uv run mypy cubebox/llm/factory.py && uv run pytest tests/e2e/test_agents.py -v
```
...
**Files:**
- Create: `backend/cubebox/tools/builtin/memory.py`

- [ ] **Step 1: Inspect an existing built-in tool (e.g., `backend/cubebox/tools/builtin/calculator.py`) for the `StructuredTool` pattern used in this repo.**

- [ ] **Step 2: Create `backend/cubebox/tools/builtin/memory.py`:**
...
- [ ] **Step 3: Wire the tools into the agent factory. In `create_cubebox_agent`, add a `memory_service_factory` parameter and call `create_memory_tools(...)` to get the three tools, then extend the `tools` list passed to `create_agent`.**

- [ ] **Step 4: Type-check.**

```bash
cd backend && uv run mypy cubebox/tools/builtin/memory.py
```
...
```bash
git add backend/cubebox/tools/builtin/memory.py backend/cubebox/agents/graph.py
...
**Files:**
- Modify: `backend/cubebox/services/memory_screen.py`
...
- [ ] **Step 3: Replace `backend/cubebox/services/memory_screen.py`:**
...
```bash
git add backend/cubebox/services/memory_screen.py backend/tests/test_memory_screen.py
...
### Task 7.1: Memory types in `@cubebox/core`
...
git commit -m "feat(memory): MemoryItem types in @cubebox/core"
```
...
```bash
git add frontend/packages/core/
git commit -m "feat(memory): MemoryApi client in @cubebox/core"
```
...
import type { MemoryScope, MemoryStatus } from "@cubebox/core";
...
import type { MemoryItem } from "@cubebox/core";
...
```bash
cd frontend && pnpm --filter @cubebox/web build && pnpm --filter @cubebox/web lint
```
...
async def test_pre_existing_malicious_workspace_memory_does_not_bypass_gate(
    ...
    from cubebox.models.memory import MemoryItem, MemoryScope, MemoryType
...
- [ ] **Step 2: If the SSE stream does not currently emit a `usage` event with provider-reported cache token counts, add it. Locate where the LLM response is consumed (likely `cubebox/agents/stream.py`) and emit a `usage` event whose payload is the raw provider usage dict. This is part of this task — the test depends on it.**
...
```bash
git add backend/tests/e2e/memory/test_prompt_cache.py backend/cubebox/agents/stream.py
...
**Plan complete and saved to `docs/superpowers/plans/2026-05-09-cubebox-memory-system.md`. Two execution options:**
