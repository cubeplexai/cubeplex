...
由 `CUBEBOX_DEPLOYMENT__MODE` env var 或 `config.yaml` 设置，启动时一次读入，挂在 FastAPI `app.state.deployment_mode`，所有 dependency / repository / route 通过 `request.app.state` 读取（便于测试 monkeypatch）。
...
实现：在 `on_after_register` 入口处取 PostgreSQL advisory lock `pg_try_advisory_xact_lock(hashtext('cubebox-singleton-org-setup'))`；拿不到锁直接抛 409。`/setup` 路径同样在事务内先取该 lock，再做 `org_count == 0` 检查。advisory lock 在事务结束自动释放，无残留风险。
...
```python
# cubebox/models/organization_membership.py
...
- `backend/cubebox/auth/dependencies.py:138` `require_org_admin`
- `backend/cubebox/api/routes/v1/admin.py:36` `/admin/me`
- `backend/cubebox/api/routes/v1/cost.py:46`
...
附带效果：multi_tenant 下从此校验 `org_id` 与用户 org-membership 一致，关闭现有 `backend/cubebox/api/routes/v1/workspaces.py:90-93` 的 `TODO(P2-auth)`。
...
- `needs_org_setup` = `mode == single_tenant && org_count == 0`，反映**系统**状态；与 `/auth/me.needs_org_setup`（**用户**状态）不同——后者还要求当前用户没有 org-membership。两者在 `cubebox admin grant-admin` 提前 bootstrap 不同用户时会发散。
...
Env override：`CUBEBOX_DEPLOYMENT__MODE=multi_tenant`。

### `cubebox admin` CLI

`backend/cubebox/cli/admin.py`，挂为 `cubebox` console_script。

```
$ cubebox admin grant-admin alice@example.com
Promoted alice@example.com to admin of org 'acme' (org_xxx).

$ cubebox admin revoke-admin alice@example.com
...
迁移 docstring 写明 backfill 规则与"脏 dev DB 上 owner 选择可能反直觉"的注意事项；操作面用 `cubebox admin grant-admin` / 未来 `transfer-ownership` 修正。
...
4. `multi_tenant_register_unchanged` —— Playwright config 翻转 `CUBEBOX_DEPLOYMENT__MODE=multi_tenant`；register → `/w/{personal}` 直落，无 `/setup`。
...
- `cubebox admin transfer-ownership <new_email>` CLI 补全。
- Reserved-slug 名单（`admin` / `api` / `app` / `www` / `cubebox`）—— slug 真正变 subdomain 时落地。
...
- `backend/cubebox/models/organization_membership.py`
- `backend/cubebox/repositories/organization_membership.py`
- `backend/cubebox/api/routes/v1/system.py`（`/system/info` + `/system/setup`）
- `backend/cubebox/cli/__init__.py`、`backend/cubebox/cli/admin.py`
...
- `backend/cubebox/config.py`（读 `deployment.mode`）
- `backend/config.yaml`、`backend/config.production.yaml`
- `backend/cubebox/auth/users.py` `on_after_register`（mode 分支 + advisory lock + 多插一行 `OrganizationMembership(role=owner)`）
- `backend/cubebox/auth/dependencies.py` `require_org_admin`
- `backend/cubebox/api/routes/v1/admin.py` `/admin/me`
- `backend/cubebox/api/routes/v1/cost.py`（admin 检查）
- `backend/cubebox/api/routes/v1/workspaces.py` `create_workspace`
- `backend/cubebox/api/routes/v1/auth.py` `/auth/me`（增加 `needs_org_setup` 字段）
- `backend/cubebox/api/app.py`（注册 system router、advisory-lock helper、startup mode-consistency 检查）
- `backend/cubebox/repositories/__init__.py`（导出新 repo）
...
- `backend/CLAUDE.md`（`cubebox admin` CLI 用法）
