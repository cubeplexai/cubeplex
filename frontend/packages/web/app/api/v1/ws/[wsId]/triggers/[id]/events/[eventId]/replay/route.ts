...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ wsId: string; id: string; eventId: string }> },
) {
  const { wsId, id, eventId } = await params

  const backendRes = await fetch(
    `${BACKEND_URL}/api/v1/ws/${wsId}/triggers/${id}/events/${eventId}/replay`,
    {
      method: 'POST',
      headers: {
