/**
 * Skills discover endpoint — GET proxy.
 * Forwards ?q=&limit= to the backend discover route.
 */
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ wsId: string }> }) {
  ...
  const backendRes = await fetch(`${BACKEND_URL}/api/v1/ws/${wsId}/skills/discover${qs}`, {
