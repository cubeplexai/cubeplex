...
export function TryItView({
  ...
  async function handleRun(): Promise<void> {
    ...
    try {
      ...
      if (surface === 'admin') {
        ...
        res = await wsInvokeTool(client, wsId, installId, toolName, args)
      }
      setResult(res)
    } catch (err) {
  ...
  const runDisabled =
    running || (surface === 'admin' && requiresWorkspacePicker === true && !scopedAdminWorkspaceId)
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {surface === 'admin' && requiresWorkspacePicker && adminWorkspaceOptions ? (
