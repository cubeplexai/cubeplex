...
export interface MessageStore {
  ...
  /** Drop all in-memory unread (and optionally clear scoped storage). */
...
function currentUnreadUserId(): string | null {
...
export const useMessageStore = create<MessageStore>((set, get) => ({
  ...
  async loadMessages(
    ...
    const existingLoad = loadMessagesInFlight.get(conversationId)
    if (existingLoad) {
    ...
    const promise = (async () => {
      try {
        ...
        // <AskUserCard> and composer hint already convey state. `pendingAsk`
        ...
        const isResumingHitl = hasPendingHitl && bootstrap.active_run?.status === 'running'
        const isPaused =
          !isResumingHitl && (bootstrap.active_run?.status === 'paused_hitl' || hasPendingHitl)
  ...
  __applyUnreadRemote(event) {
    ...
    // type === 'replace' (storage fallback): merge carefully, strip present focus.
    let next = event.ids
    const activeId = useConversationStore.getState().activeId
    if (activeId && next[activeId] && !isAwayFromConversation(activeId)) {
      next = { ...next }
