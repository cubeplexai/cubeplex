...
import { cancelActiveRun, cancelSteer, getConversationBootstrap, steerRun, streamMessages, streamRun, } from '../api';
...
function emptyStream(name = null) {
...
function nextEventId(current, next) {
    if (!next)
        return current;
    if (!current)
        return next;
 ...
 */
function normalizeThinkingTiming(blocks) {
    return blocks.map((b) => {
        if (b.type !== 'thinking' || b.started_at == null)
            return b;
...
function appendThinkingBlock(blocks, delta, startedAt) {
    const last = blocks[blocks.length - 1];
...
function appendTextBlock(blocks, delta) {
    const last = blocks[blocks.length - 1];
...
function parseTodosFromToolCall(args) {
...
function buildPendingUserMessage(runId, content) {
    return {
        id: `pending-${runId}`,
        role: 'user',
 ...
 */
export function trimHistoryForActiveRun(messages, runId, content, startedAt) {
...
function restoreTodosFromHistory(messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
        const msg = messages[i];
...
function hydrateCitationsFromHistory(conversationId, messages) {
...
function applyStreamEvent(state, event) {
    ...
    if (event.type === 'tool_call') {
        ...
        const nextTodos = e.data.name === 'write_todos' ? parseTodosFromToolCall(e.data.arguments) : state.todos;
        return {
            ...base,
            todos: nextTodos,
            toolStartedMap: {
                ...state.toolStartedMap,
                [e.data.tool_call_id]: existingStartedAt ?? timestampToMs(e.data.started_at ?? event.timestamp),
            },
            streamAgents: {
                ...state.streamAgents,
                [agentKey]: {
                    ...
                    blocks: appendToolCallBlock(prev.blocks, e.data.name, e.data.arguments, e.data.tool_call_id),
                },
            },
        };
    }
    if (event.type === 'tool_call_delta') {
        ...
        const nextToolStartedMap = e.data.tool_call_id && !state.toolStartedMap[e.data.tool_call_id]
            ? {
                ...state.toolStartedMap,
                [e.data.tool_call_id]: startedAt,
            }
            : state.toolStartedMap;
        const existingIdx = blocks.findIndex((block) => block.type === 'tool_call_streaming' && block.index === idx);
        if (existingIdx >= 0) {
    ...
    if (event.type === 'tool_result') {
        ...
        if (tcId) {
            newMap[tcId] = {
                ...
                startedAt: state.toolStartedMap[tcId] ??
        ...
        const clearAsk = e.data.name === 'ask_user' ? { pendingAsk: null } : {};
        return {
            ...base,
            ...clearAsk,
    ...
    if (event.type === 'sandbox_confirm_resolved') {
        const d = event.data;
        const tcId = Object.entries(state.pendingConfirmMap).find(([, v]) => v.question_id === d.question_id)?.[0];
        if (!tcId)
            return base;
    ...
    if (event.type === 'ask_user_request') {
        const d = event.data;
        if (state.pendingAsk?.question_id === d.question_id)
 ...
 */
function buildTurnMessages(agents, toolResultMap, turnUsage, stopReason = 'stop') {
    const mainStream = agents[MAIN_AGENT_KEY];
    if (!mainStream)
        return { assistantMessage: null, toolMessages: [] };
    const finalBlocks = finalizeLastThinking(mainStream.blocks).filter((block) => block.type !== 'tool_call_streaming');
    ...
    for (const tr of mainStream.toolResults) {
        ...
        if (!tcId || tr.data.tool_name === 'subagent')
            continue;
        const mapEntry = toolResultMap[tcId];
        const receivedAtMs = mapEntry?.receivedAt ?? (tr.timestamp ? new Date(tr.timestamp).getTime() : Date.now());
        toolMessages.push({
...
async function finalizeCompletedStream(get, set, conversationId, stopReason = 'stop') {
    const { assistantMessage, toolMessages } = buildTurnMessages(get().streamAgents, get().toolResultMap, get().turnUsage[conversationId] ?? null, stopReason);
    if (!assistantMessage) {
        set((state) => ({
            isStreaming: false,
            pendingConfirmMap: {},
...
async function consumeRunStream(client, conversationId, runId, lastEventId, set, get, signal) {
    ...
    try {
        for await (const event of streamRun(client, conversationId, runId, lastEventId, signal)) {
            ...
            if (event.event_id && state.lastAppliedEventId) {
                if (compareEventIds(event.event_id, state.lastAppliedEventId) <= 0)
                    continue;
            }
...
export const useMessageStore = create((set, get) => ({
    ...
    sessionUsage: {},
    contextWindow: {},
    async loadMessages(client, conversationId) {
        const state = get();
        if (state.isStreaming && state.streamingConversationId === conversationId)
            return;
        try {
            const bootstrap = await getConversationBootstrap(client, conversationId);
            const current = get();
            if (current.isStreaming && current.streamingConversationId === conversationId)
                return;
            let messages = normalizeMessages(bootstrap.messages ?? []);
            if (bootstrap.active_run?.user_message) {
                messages = trimHistoryForActiveRun(messages, bootstrap.active_run.run_id, bootstrap.active_run.user_message, bootstrap.active_run.started_at ?? null);
            }
            ...
            const newTurnUsage = {
                ...get().turnUsage,
                [conversationId]: (usageSummary?.turn ?? null),
            };
            const newSessionUsage = {
                ...get().sessionUsage,
                [conversationId]: (usageSummary?.session ?? null),
            };
            const newContextWindow = {
                ...get().contextWindow,
                [conversationId]: usageSummary?.context_window ?? null,
            };
            const nextStreamAgents = bootstrap.active_run
            ...
            if (bootstrap.active_run) {
                ...
                queueMicrotask(() => {
                    void consumeRunStream(client, conversationId, runId, undefined, set, get, controller.signal).finally(() => {
    ...
    async send(client, conversationId, content, attachmentIds, attachments) {
        ...
        activeStreamController = controller;
        let retried = false;
        let streamSource = streamMessages(client, conversationId, content, attachmentIds, controller.signal);
        let processed = 0;
        try {
            outer: for (;;) {
                for await (const event of streamSource) {
                    if (event.event_id && get().lastAppliedEventId) {
                        if (compareEventIds(event.event_id, get().lastAppliedEventId) <= 0)
                            continue;
                    }
                    if (event.type === 'artifact') {
                        const artifactData = event.data;
                        ...
                        if (!retried && !sawDone && errData.message.includes('409')) {
                            ...
                            streamSource = streamMessages(client, conversationId, content, attachmentIds, controller.signal);
                            continue outer;
                        }
                        set((s) => ({
                            error: errData.details || errData.message,
    ...
    async steer(client, conversationId, content) {
        ...
        const removePending = () => set((s) => ({
            pendingSteers: {
                ...s.pendingSteers,
                [conversationId]: (s.pendingSteers[conversationId] ?? []).filter((p) => p.steerId !== steerId),
            },
        }));
        try {
    ...
    __commitTurnAndInject(conversationId, data) {
        ...
        const already = (state.messages[conversationId] ?? []).some((m) => m.role === 'user' && m.metadata?.steer_id === data.steer_id);
        if (already)
            return;
        const { assistantMessage, toolMessages } = buildTurnMessages(state.streamAgents, state.toolResultMap, state.turnUsage[conversationId] ?? null);
        ...
        set((s) => ({
            messages: {
                ...
                [conversationId]: [
                    ...
                    ...(mainHasContent ? [assistantMessage] : []),
                    ...toolMessages,
                    steerMessage,
                ],
            },
            ...
            pendingSteers: {
                ...s.pendingSteers,
                [conversationId]: (s.pendingSteers[conversationId] ?? []).filter((p) => p.steerId !== data.steer_id),
            },
        }));
    },
    ...
    __applyEvent(event) {
        set((s) => applyStreamEvent(s, event));
    },
}));
