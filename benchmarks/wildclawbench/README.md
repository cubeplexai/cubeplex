# cubebox × WildClawBench harness

Drive cubebox over its HTTP API on [WildClawBench](https://github.com/internlm/WildClawBench)'s
...
cubebox's harness extracts ≥ their capability.
...
- `wcb_harness/transcript.py` — cubebox SSE → OpenClaw-JSONL transcript. **Verified
  against a real cubebox SSE trace: their `load_transcript` reads it, and
...
- 27/60 tasks declare skills; the dominant one is `agent-browser` (an npm CLI +
  SKILL.md, installed via a `npm install -g agent-browser` warmup) — cubebox-friendly
  (warmup installs the tool, cubebox's SkillsMiddleware loads the SKILL.md).
- Grading is just "run the task's `grade(transcript, workspace_path)` in an env that
  ...
  cubebox's sandbox (on the WildClawBench image) IS such an env → grading can run
  **in the same sandbox via cubebox `execute`**, no second container. See design §3.

Pending (need the downloaded images / a running cubebox):
- Phase 1: load+inspect `wildclawbench-ubuntu`/`-hermes` images (execd bake-in,
  bundled-skill layout, deps), push to registry, smoke-test cubebox on the image.
...
CUBEBOX_SSE=<path/to/any/non-empty/sse.jsonl> \
...
benchmarks/wildclawbench/
  wcb_harness/
    ...
    transcript.py   cubebox SSE → OpenClaw JSONL (verified)
